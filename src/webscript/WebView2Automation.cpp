...
bool WebView2Automation::Click(const std::string& selector, std::string* error) {
    ...
    if (!ExecuteScriptSync(script, result)) {
        if (error) *error = "ExecuteScript failed";
        return false;
    }
...
void WebView2Automation::Sleep(int ms) {
...
WebView2Automation::ActionResult WebView2Automation::DoAction(const std::string& title, std::string* outMessage) {
    // 转义单引号
    ...
    if (!ExecuteScriptSync(script, result)) {
        if (outMessage) *outMessage = "ExecuteScript failed";
        return ACTION_EXCEPTION;
    }

    json j;
    if (!ParseJsonResult(result, j)) {
        if (outMessage) *outMessage = "JSON parse failed: " + result;
        return ACTION_EXCEPTION;
    }
    ...
    if (j.contains("errcode")) {
        ...
        return static_cast<ActionResult>(errcode);
    }
