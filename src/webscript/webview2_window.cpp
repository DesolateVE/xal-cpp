...
bool WebView2Window::CreateWindowInternal()
{
    ...
    // 创建窗口（无标题栏/边框，禁止调整大小）
    DWORD style = m_params.borderless ? WS_POPUP : WS_OVERLAPPEDWINDOW; // 移除 WS_THICKFRAME 禁止调整大小
...
{
    // 构建 userDataFolder 到 LocalAppData\MSLoginWebView2
    ...
    CreateCoreWebView2EnvironmentWithOptions(
        ...
        Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>([this](HRESULT result, ICoreWebView2Environment *env) -> HRESULT
            ...
            // 如果启用了 InPrivate 模式，使用 ControllerOptions
            if (m_params.privateMode) {
                        ...
                        env10->CreateCoreWebView2ControllerWithOptions(
                            ...
                            Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                                [this](HRESULT result, ICoreWebView2Controller* controller) -> HRESULT {
                                    return InitializeController(result, controller);
                                }
                            ).Get()
                        );
                        return S_OK;
            ...
            if (payload && pWindow->m_webview)
            {
                pWindow->m_webview->ExecuteScript(
                    ...
                    Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
                        [payload](HRESULT error, LPCWSTR resultObjectAsJson) -> HRESULT
                        {
                            std::unique_lock<std::mutex> lock(payload->mtx);
                            payload->hr = error;
            ...
            if (payload && pWindow->m_webview)
            {
                HRESULT hr = pWindow->m_webview->AddScriptToExecuteOnDocumentCreated(
                    ...
                    Callback<ICoreWebView2AddScriptToExecuteOnDocumentCreatedCompletedHandler>(
                        [payload](HRESULT error, PCWSTR id) -> HRESULT
                        {
                            std::unique_lock<std::mutex> lock(payload->mtx);
                            payload->hr = error;
                            payload->done = true;
